...
export function bindSoundState({ canPlaySoundSelector, getSimSpeedValue }) {
	if (typeof canPlaySoundSelector === "function") {
		canPlaySound = canPlaySoundSelector;
	}
	if (typeof getSimSpeedValue === "function") {
		getSimSpeed = getSimSpeedValue;
	}
}
...
export const soundManager = {
	...
	playSound(type, scale = 1) {
		...
		// *all* the way back.
		if (!canPlaySound() || getSimSpeed() < 0.95) {
			return;
		}
		...
		if (type === "burstSmall") {
			const now = Date.now();
			const perfScale = perfTuning.enabled ? Math.max(perfTuning.scale, 0.5) : 1;
			const minInterval = Math.round(20 / perfScale);
